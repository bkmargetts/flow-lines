/**
 * Curated botanical ink palettes for the Vine Generator. Each maps the module's
...
export const VINE_PALETTES: VinePalette[] = [
...
export function getVinePalette(id: string): VinePalette | null {
  if (id === CUSTOM_PALETTE) return null;
  return VINE_PALETTES.find((p) => p.id === id) ?? VINE_PALETTES[0];
}
