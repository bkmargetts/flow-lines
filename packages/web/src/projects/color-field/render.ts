...
function colorFieldToOptions(state: ColorFieldState, page: PageMetrics, marginPx: number): ColorFieldOptions {
  ...
  return {
    ...
    bandCount: state.colorCount,
    bandWaveAmpPx: state.bandWaveAmpMm * ppm,
    bandWaveLengthPx: state.bandWaveLengthMm * ppm,
    featherPx: state.featherMm * ppm,
    featherNoiseScale: state.featherNoiseScale,
    densityGradient: state.densityGradient,
    densityNoiseAmt: state.densityNoiseAmt,
    densityNoiseScale: state.densityNoiseScale,
